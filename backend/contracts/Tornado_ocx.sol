...
contract Tornado is ReentrancyGuard {
    ...
    uint256[] public denominations = [1 ether, 2 ether, 5 ether, 10 ether, 20 ether, 50 ether, 100 ether, 200 ether, 500 ether, 1000 ether];
    ...
    mapping(address => uint256) public userDeposits; // Store user deposits
    ...
    event Deposit(uint256 root, uint256[10] hashPairings, uint8[10] pairDirection, uint256 amount);
    ...
    function deposit(uint256 _commitment) external payable nonReentrant {
        ...
        userDeposits[msg.sender] += msg.value; // Track deposit per user

        emit Deposit(newRoot, hashPairings, hashDirections, msg.value);
    }
    ...
    function withdraw(
        ...
        uint256 withdrawalAmount = userDeposits[msg.sender];
        require(withdrawalAmount > 0, "no-deposit");

        nullifierHashes[_nullifierHash] = true;
        userDeposits[msg.sender] = 0; // Reset deposit after withdrawal

        address payable target = payable(msg.sender);

        (bool ok, ) = target.call{ value: withdrawalAmount }("");
        require(ok, "payment-failed");

        emit Withdrawal(msg.sender, _nullifierHash, withdrawalAmount);
    }
}
